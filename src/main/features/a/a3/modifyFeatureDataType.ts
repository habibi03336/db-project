  ...
  ipcMain.handle(channelName, async (event, arg) => {
    try {
      ...
      if (numericTypes.has(res[0].data_type)) {
        ...
        // if it is a primary key, it can't be converted to text
        if (res4.length !== 0) {
          res4.forEach(async (item) => {
            if (item.CONSTRAINT_NAME === 'PRIMARY') {
              return error('can not convert primary key to text');
            }
            // drop the foreign key constraint
            const dropFK = `ALTER TABLE ${modTable} DROP CONSTRAINT ${item.CONSTRAINT_NAME}`;
            await dbClient.sql(dropFK);
          });
        }
        ...
        // restore the constraint if the column is a foreign key
        res4.forEach(async (item) => {
          ...
          if (constraintName !== 'PRIMARY') {
            const restoreFK = `ALTER TABLE ${modTable} ADD CONSTRAINT ${constraintName} FOREIGN KEY (${colName}) REFERENCES ${refSchema}.${refTable}(${refCol})`;
            await dbClient.sql(restoreFK);
          }
        });
        ...
        const isAllInteger = res3.every((item) => {
          return Number.isInteger(item[colName]);
        });
        ...
        if (isAllInteger) {
          ...
          if (res6.length !== 0) {
            res6.forEach(async (item) => {
              if (item.CONSTRAINT_NAME === 'PRIMARY') {
                return error('can not convert primary key to numeric');
              }
